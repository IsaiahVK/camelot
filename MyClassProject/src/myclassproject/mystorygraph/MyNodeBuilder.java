...
public class MyNodeBuilder extends NodeBuilder {
	...
	public void askForAdviceActions() {
		...
		node.add(new HideDialog()).add(new DialogSequence(player, knight, List.of("It takes many years of training to hone your skills young one. If you can prove your worth you can catapault through the ranks."),
				List.of("Thank you!")));
	}
	//David Paul
	@BuilderMethod
	...
	public void thankTheKnightActions() {
		...
		node.add(new HideDialog()).add(new DialogSequence(player, knight, List.of("Oh you're welcome. Now move along."), List.of("Yes sir!")));
	}
	...
	public void atTownSquareActions() {
		...
		.add(new EnableInput()).add(new CreateCharacterSequence(friend1)).add(new SetPosition(friend1, townSquare, "EastEnd"))
		.add(new Face(friend1, player));
	}
	@BuilderMethod
	public void friendTalkActions() {
		var node = get(MyNodeLabels.friendTalk.toString());
		node.add(new DialogSequence(player, friend1, List.of("Hey! Are you ready for the knight academy?"), List.of("So excited!")));
	}
	@BuilderMethod
	public void friendResponseActions() {
		var node = get(MyNodeLabels.friendResponse.toString());
		node.add(new HideDialog()).add(new DialogSequence(player, friend1, List.of("What house did you get?"), List.of("Veylthar Manor", "Solara Keep")));
	}
	@BuilderMethod
	public void velytharManorActions() {
		var node = get(MyNodeLabels.velytharManor.toString());
		node.add(new HideDialog()).add(new DialogSequence(player, friend1, List.of("Velythar Manor is the house of magicians and prophets. Harry Houdini founded the house to continue his legacy. We get to be roommates and embark on this journey together!"), List.of("See you later! I'm going to get my key.")));
	}
	@BuilderMethod
	public void solaraKeepActions() {
		var node = get(MyNodeLabels.solaraKeepActions.toString());
		node.add(new HideDialog()).add(new DialogSequence(player, friend1, List.of("Solara Keep is the house of aspiring knights and war mongers. Warriors of the Keep are known to be the protectors of these lands. I'm in Veylthar Manor."), List.of("Oh it sucks so much that we can't be roommates. I guess I will see you later. I'm heading to grab my key.")));
	}
	@BuilderMethod
	...
	@BuilderMethod
	public void ladyTalkActions() {
		var node = get(MyNodeLabels.ladyTalk.toString());
		node.add(new DialogSequence(player, oldLady, List.of("Here is your room key, young lad. Enjoy your stay."), List.of("Thank you, ma' am.")))
	...
	public void scrollDoneActions() {
		...
		.add(new Face(friend2, player));
	}
	@BuilderMethod
	public void wingItActions() {
		var node = get(MyNodeLabels.wingIt.toString());
		node.add(new DialogSequence(player, friend2, List.of("Do you want to eat in the dining hall or go to the club fair?"), List.of("I'm hungry!", "I want to see some clubs.")));
	}
	@BuilderMethod
	public void atDiningHallActions() {
		var node = get(MyNodeLabels.atDiningHall.toString());
		node.add(new HideDialog()).add(new DisableInput()).add(new Exit(player, bedroomDoor, true)).add(new Enter(player, diningHallDoor, true))
		.add(new WalkTo(player, diningHall, "Table")).add(new EnableInput())
		.add(new NarrationSequence("You arrive at the dining hall and you have never smelled anything so good! You look at the labels for the dishes and decide to eat one."));
	}
	
	@BuilderMethod
	public void chooseEatActions() {
		var node = get(MyNodeLabels.chooseEat.toString());
		node.add(new HideNarration()).add(new SetPosition(roastedBoar, diningTable)).add(new SetPosition(meatPies, diningTable)).add(new SetPosition(dragonfireDumpling,diningTable));
	}
	@BuilderMethod
	public void eatDumplingActions() {
		...
		.add(new NarrationSequence("This is the best food you have ever had. You have never felt more alive and nourished. Your skin glows radiently for a moment. Perhaps this food has magical properties."));
	}
	@BuilderMethod
	...
	public void eatPieActions() {
		...
		node.add(new Take(player, meatPies)).add(new Drink(player)).add(new NarrationSequence("This is the best food you have ever had. You have never felt more alive and nourished. Your skin glows radiently for a moment. Perhaps this food has magical properties."));
	}
	@BuilderMethod
	public void diningToClubActions() {
		var node = get(MyNodeLabels.diningToClub.toString());
		node.add(new HideNarration()).add(new DisableInput()).add(new Exit(player, diningHallDoor, true))
		.add(new Enter(player, clubFairDoor, true))
		.add(new CreateCharacterSequence(friend3)).add(new SetPosition(friend3, clubFair, "Supplicant"))
		.add(new SetPosition(friend2, clubFair, "LeftFoyer"))
		.add(new WalkTo(player, clubFair, "LeftSupplicant")).add(new WalkTo(friend2, clubFair, "LeftSupplicant"))
		.add(new EnableInput());
	}
	@BuilderMethod
	public void bedroomToClubActions() {
		var node = get(MyNodeLabels.bedroomToClub.toString());
		node.add(new HideDialog()).add(new DisableInput()).add(new Exit(player, bedroomDoor, true))
		.add(new Enter(player, clubFairDoor, true)).add(new CreateCharacterSequence(friend3))
		.add(new SetPosition(friend3, clubFair, "Supplicant")).add(new SetPosition(friend2, clubFair, "LeftFoyer"))
		.add(new WalkTo(player, clubFair, "LeftSupplicant")).add(new WalkTo(friend2, clubFair, "LeftSupplicant"))
		.add(new EnableInput());
	}
	...
	public void atClubFairActions() {
		...
		node
		.add(new DialogSequence(player, friend3, List.of("Hey! Want to join the archery club? It's a ton of fun!"),
	...
	public void backToBedActions() {
		...
		node.add(new HideDialog()).add(new DisableInput()).add(new Exit(player, clubFairDoor, true))
	...
	public void atHallwayActions() {
		...
		.add(new SetPosition(player, hallway, "Stairs"))
		.add(new DialogSequence(player, player, List.of("Envision your future"), List.of("Blade of Chaos", "Wand of Primordial Passion")))
		.add(new CreateCharacterSequence(knight2))
		.add(new SetPosition(knight2, hallway, "Center")).add(new Face(knight2, player));
	}
	@BuilderMethod
	public void bladeOfChaosActions() {
		var node = get(MyNodeLabels.bladeOfChaos.toString());
		node.add(new HideDialog())
		.add(new NarrationSequence("Wielded by Kratos Ghost of Sparta, this blade is limitless in its destructive capability. It takes a strong will to put them down when the job is done."));
	}
	@BuilderMethod
	public void wandOfPrimordialPassionActions() {
		var node = get(MyNodeLabels.wandOfPrimordialPassion.toString());
		node.add(new HideDialog())
		.add(new NarrationSequence("This legendary wand was used by Merlin himself to purge the dark forces from the lands. This wand allows you to control the four elements of reality as we know it: fire, lightning, water, and earth."));
	} 
	@BuilderMethod
	public void useBladeActions() {
		var node = get(MyNodeLabels.useBlade.toString());
		node.add(new HideNarration()).add(new Draw(player, bladeOfChaos))
		.add(new DisableInput()).add(new WalkTo(player, knight2))
		.add(new Attack(player, knight2, true))
		.add(new Attack(player, knight2))
	...
	public void endGame() {
		...
		node.add(new FadeOut()).add(new ShowMenu()); 
	}
